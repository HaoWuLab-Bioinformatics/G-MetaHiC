...
import torch
# sys.modules['scipy.sparse._csr'] = scipy.sparse.csr_matrix.__class__.__module__
...
def get_metacell_profile(sc_atac_dict, nbrs):
    ...
    for chrom in list(sc_atac_dict.keys()):
        print(sparse.csr_matrix(metacell).shape)
        print(sc_atac_dict[chrom].shape)
...
def load_hic(data_path, ct):
    zscore_dict = pickle.load(
        ...
        open(os.path.join(data_path, "hic/zvalue/{}/hic_zscore_dict.p".format(ct)), "rb")
    )
    # qvalue_dict = pickle.load(
    #     open(os.path.join(data_path, "hic/qvalue/{}/hic_qvalue_dict.p".format(ct)), "rb")
    # )
    qvalue_dict = pickle.load(
        # open(os.path.join(data_path, "hic/{}_hic_zscore_dict.p".format(ct)), "rb")
        open(os.path.join(data_path, "hic/zvalue/{}/hic_zscore_dict.p".format(ct)), "rb")
...
def load_atac(data_path, ct, pbulk_only=False):
    bulk_atac_dict = pickle.load(
        open(os.path.join(data_path, "atac/{}/{}_tile_50bp_dict.p".format(ct,ct)), "rb")

    )

    # 本实验使用伪批量sc_atac_seq数据和元细胞的共可及性以及ctcf_score预测hic接触图
    # pbulk_only表示只使用伪批量sc_atac_seq数据
    if pbulk_only:
        sc_atac_dict = None
    else:
        sc_atac_dict = pickle.load(
            open(os.path.join(data_path, "atac/{}/{}_tile_500bp_dict.p".format(ct,ct)), "rb")

        )
        metacell_mask = pd.read_csv(
            os.path.join(data_path, "atac/{}/{}_metacell_mask.csv".format(ct, ct)), index_col=0
            # os.path.join(data_path, "atac/{}/{}_metacell_mask_GB_use.csv".format(ct,ct)), index_col=0
        ).values
...
def load_atac_eval(data_path, ct, pbulk_only=False):
    bulk_atac_dict = pickle.load(
        open(os.path.join(data_path, "atac/{}/{}_tile_pbulk_50bp_dict.p".format(ct,ct)), "rb")
    )
...
def normalize_bulk_dict(pbulk_dict, effective_genome_size):
    ...
    scale_factor = (libsize * 150) / (effective_genome_size)
    # scale_factor = (libsize * 100) / (effective_genome_size)
    for chrom in pbulk_dict.keys():
