...
python gmetahic/scATAC_preparation.py \
...
parser.add_argument("--fragment_file", help="Path and name of the input fragment file.",
                    type=str, default="../gmetahic/datasets/atac/gm12878/gm12878_fragments_1.tsv.gz")
parser.add_argument("--barcode_file", help="Path and name of the valid barcode file.",
                    type=str, default="../gmetahic/datasets/atac/gm12878/archr_filtered_barcode.csv")
parser.add_argument("--genome_assembly", help="Genome assembly of the scATAC files.",
                    type=str, default="hg38")
parser.add_argument("--lsi_file", help="Path and name of the LSI file from ArchR running.",
                    type=str, default="../gmetahic/datasets/atac/gm12878/archr_filtered_lsi.csv")
parser.add_argument("--save_path", help="Path to save all the intermediate and final files.",
                    type=str, default="../gmetahic/datasets")
...
chrom_size = pd.read_csv('./gmetahic/datasets/genome/hg38_chrNameLength.txt', #downloaded from UCSC
...
def atac_processing(frag_path, nrows_skip, valid_barcode_path,
    ...
    for chrom in list(set(frag_stacked[0])):
        if chrom in valid_chrom:
            ...
            print(chrom)
    #2c. save to file
    pickle.dump(tile_dict, open(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_tile_500bp_dict.p", "wb"))
    np.save(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_tile_500bp_barcode.npy", barcodes.astype(str))
    ...
    for chrom in list(set(frag_stacked[0])):
        if chrom in valid_chrom:
            ...
            tile_dict[chrom] = frag_grouped
    #3b. save
    pickle.dump(tile_dict, open(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_tile_pbulk_50bp_dict.p", "wb"))
    del frag_stacked, tile_dict
    ...
    for chrom in valid_chrom:
        ...
        tile_dict[chrom] = frag_grouped
    #4c. save
    pickle.dump(tile_dict, open(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_tile_50bp_dict.p", "wb"))  # save it into a file named save.p
    np.save(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_tile_50bp_barcode.npy",barcodes.astype(str))
    del frag_stacked, tile_dict
...
def metacell_computing(lsi_path, n_neighbors = 100, max_overlap = 33,
    ...
    pd.DataFrame(metacell_assignment).to_csv(f"{save_path}/atac/{cell_type_prefix}/{cell_type_prefix}_metacell_mask.csv")
    return None
...
def main():
    ...
    #                      sep = '\t', header = None, index_col = 0)
    chrom_size = pd.read_csv('./gmetahic/datasets/genome/hg38_chrNameLength.txt',
                             sep='\t', header=None, index_col=0)
