...
get_chr_sizes<-function(gen="Hsapiens",gen_ver="hg38",chrs=NULL){
...
get_chrs<-function(gen="Hsapiens",gen_ver="hg38"){
  genome <- paste("BSgenome.", gen, ".UCSC.", gen_ver, sep = "")
  library(genome, character.only = TRUE)
...
hic2hicdc <-
  function(hic_path,
    ...
    if (base::grepl(".txt", bintolen_path, ignore.case = TRUE)) {
      ...
      stop("bintolen_path must be .txt or .rds")
    }

    bintolen <- bintolen %>%
      tidyr::separate(
        "bins", c("chr", "start", "end"),
        sep = "-", remove = FALSE, convert = TRUE, extra = "drop", fill = "warn"
      ) %>%
    ...
    if (any(!is.finite(size))) {
      ...
      size[miss] <- as.numeric(derived[chrom_plain[miss]])
    }

    if (any(!is.finite(size))) {
      stop("Missing chromosome sizes for: ",
           paste(chrom_plain[!is.finite(size)], collapse = ", "))
    }
    ...
    do_straw_intra <- function(chr_plain) {
      ...
      if (inherits(res, "try-error") || is.null(res) || nrow(res) == 0) {
        res <- straw(
          ...
          fname   = path.expand(hic_path),
          binsize = binsize,
          chr1loc = paste0("chr", chr_plain),
          chr2loc = paste0("chr", chr_plain),
          unit    = "BP"
        )
      }
      res
    ...
    do_straw_pair <- function(a_plain, b_plain) {
      res <- try(straw(
        norm    = "NONE",
        fname   = path.expand(hic_path),
        binsize = binsize,
      ...
      if (inherits(res, "try-error") || is.null(res) || nrow(res) == 0) {
        res <- straw(
          ...
          fname   = path.expand(hic_path),
          binsize = binsize,
          chr1loc = paste0("chr", a_plain),
          chr2loc = paste0("chr", b_plain),
          unit    = "BP"
        )
      }
      res
    ...
    for (chr in chrom_plain) {
      ...
      bintolen_chr <- bintolen_chr %>%
        tidyr::replace_na(list(gc = 0, map = 0, len = 0, chr = chr)) %>%
        dplyr::mutate(
      ...
      index1  <- unlist(sapply(1:numbins, function(x) rep(x, min(maxbins + 1, numbins - x + 1))))
      index2  <- unlist(sapply(1:numbins, function(x) seq(x, min(x + maxbins, numbins), 1)))
      bintolen_chr <- dplyr::bind_cols(
        (bintolen_chr %>% dplyr::rename_all(function(x) paste0(x, "I")))[index1, ],
        (bintolen_chr %>% dplyr::rename_all(function(x) paste0(x, "J")))[index2, ]
      )
      rm(index1, index2)

      bintolen_chr <- bintolen_chr %>%
        dplyr::mutate(D = abs(.data$midI - .data$midJ)) %>%
        dplyr::filter(.data$D <= Dthreshold)
      ...
      count_matrix <- count_matrix %>%
        dplyr::mutate(chrI = chr, chrJ = chr)

      bintolen_chr <- dplyr::left_join(bintolen_chr, count_matrix) %>%
      ...
      if (inter) {
        ...
        count_matrix <- NULL
        for (chr2 in chrom_plain) {
          if (chr2 != chr) {
            count_matrix_add <- unique(do_straw_pair(chr, chr2))
        ...
        count_matrix <- count_matrix %>%
          ...
          dplyr::summarize(inter = sum(.data$inter), .groups = "drop") %>%
          dplyr::mutate(start = .data$start)

        bintolen_chr <- dplyr::left_join(
          bintolen_chr,
          count_matrix %>% dplyr::rename("startI" = "start", "interI" = "inter")
        ) %>% tidyr::replace_na(list(interI = 0))

        bintolen_chr <- dplyr::left_join(
          bintolen_chr,
          count_matrix %>% dplyr::rename("startJ" = "start", "interJ" = "inter")
      ...
      data.table::fwrite(
        bintolen_chr, filepath,
        quote = FALSE, sep = "\t", row.names = FALSE
      )
      rm(bintolen_chr)
      gc()
      message("Chromosome ", chr, " processed.")
...
hicdcplus_model=function(file="matrix.rds",
  ...
  remove_outliers_nb <- function(dat, mod) {
    ...
    new.dat <- dat %>% dplyr::filter(.data$counts <= q) %>% dplyr::select(-q)
    return(new.dat)
  }
...
hicdc2hic<-function(input_path,jarfile,
                    binsize=5000,gen="Hsapiens",gen_ver="hg38",mode='zvalue',
  ...
  for (chr in chrs){
    ...
    norm_data <- norm_data %>%
      dplyr::mutate(
        ...
        chrJ_num = as.numeric(factor(chrJ, levels = c(1:22, "X")))
      ) %>%
      ...
      dplyr::mutate(
        ...
        temp_startJ = ifelse(chrI_num > chrJ_num, startI, startJ)
      ) %>%
      dplyr::mutate(
        chrI = temp_chrI,
        chrJ = temp_chrJ,
  ...
  combined_data <- data.table::rbindlist(all_data) %>%
    ...
    dplyr::arrange(chrI_num, chrJ_num, startI, startJ) %>%
